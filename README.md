...
- **A comparison page** — tick "Compare" on up to four rooms and `/compare` lines them up: rent, distance and commute from your chosen campus, and a tick-or-dash for each of the nine things rooms are compared on (private toilet, kitchen inside, parking, own Luku meter, water included, water tank, fenced compound, furnished, internet). The cheapest, nearest and most complete room in each row is flagged. From there you can open a room, request a viewing, swap the campus, drop a room, add another, or pick from **rooms like these** — the ones closest on rent and distance. The URL carries the rooms, so a comparison can be sent to someone
...
- `public/compare.html` — side-by-side comparison plus similar rooms
